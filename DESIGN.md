...
- Every `servizi/*.html` opens with a **two-sheet stage** (`.stage-seq`), not a hero with a video column. Two layers share one grid cell inside a `position: sticky` pin: the video sits underneath, present from load; the hero is an opaque white sheet on top. Scrolling lifts the sheet and uncovers the video — the page itself never moves under the visitor (no scroll-jacking, same rule as the horizontal roadmap).
...
- **Only `agente-vocale` and `chatbot` have their video files today.** The other nine declare `../assets/video/<slug>.mp4` and fall back to `is-video-missing`: a Surface Alt frame with a mono "Video dimostrativo, in arrivo" caption, both controls hidden — the same honesty already applied to og-image, Calendly and the placeholder P.IVA.
