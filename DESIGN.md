...
### Hero Shader Field (home + service pages)

- All eleven `servizi/*.html` heroes and the homepage hero (`index.html`, `.hero`) sit on a WebGL field (`js/hero-shader.js`, mounted on `[data-hero-shader]`) instead of flat Surface. This is a **deliberate, recorded exception** to "flat by default" and to `MOTION_SYSTEM.md` §4.4's ban on a camera that moves by itself — not drift. The one regression this project has already reverted twice (a second accent hue) survived precisely because nobody wrote the decision down; this paragraph is that record.
- **The palette is bounded by contrast, not by taste.** The field runs from Surface to a single darkest stop at 20% Instrument Blue over white (`#CCDBE5`). That ceiling is derived, not chosen: the weakest text over the field is the `.lede` in Ink Dim (`#5A5A5A`), which needs a background at relative luminance ≥ 0.635 to clear 4.5:1. The darkest stop sits at 0.691 (**4.87:1**), and at the grain's worst excursion still 0.645 (**4.57:1**). Contrast is therefore guaranteed by construction at every pixel of every frame — there is no white scrim under the text, and so no scrim that can fail. Darkening any of the four stops means redoing that arithmetic first.
- **The grain is structural, not decoration.** Across a luminance range this narrow an 8-bit gradient bands visibly; the grain is the dither that prevents it. It fades out with the field over the bottom 16% so the hero's hairline seam still lands on clean white.
- **No cursor reactivity.** The upstream component's pointer response was removed outright — uniforms, GLSL branch, and its four window listeners, among them a capturing `scroll` handler calling `getBoundingClientRect()` on every event. On the four stage pages that scroll already drives the sticky pin.
- **On the four video-stage pages the field is the sheet itself.** The canvas mounts inside `.stage-layer--hero`, not behind the pin, and renders with `alpha: false` so the sheet stays opaque: the lift that uncovers the video is unchanged. The shader is the paper, not something behind it.
- **The homepage hero keeps its own `overflow:hidden`.** Unlike the service-page heroes, `.hero` clips its canvas so the field respects the white shell's 40px rounded bottom corners; the CTA inside never sits close enough to that edge to lose its focus ring.
- **The homepage no longer shows the topographic map in its top hero.** `data-topo` (the hover-revealed contour map from `js/main.js`) was removed from `.hero` when the shader replaced it there, so the two canvases still never draw in the same frame — the topo map continues to live only in the closing `cta-band` of every service page.
- `prefers-reduced-motion` draws one frame and never starts the rAF loop — the poster rule of `MOTION_SYSTEM.md` §5. Without WebGL the canvas stays empty and the parent's Surface (or, on the home, its existing gradient) renders the previous hero exactly.
...
### Roadmap Walker (homepage)

- A `<sprite-mate>` — the same pixel mascot the chat widget uses — **walks the homepage roadmap's dashed curve**, pausing on each of the five points before moving to the next, reversing at the end, in a loop. This is the **second registered exception** to `MOTION_SYSTEM.md` §4.4's ban on anything that moves by itself, after the Hero Shader Field. It is not a new self-moving element: the same mascot previously patrolled the services section's cut-out on a 6s infinite loop, and moved here when that section was replaced by the homepage FAQ. It arrived with a stricter contract than the one it had.
- **It walks on the line, not near it.** Position comes from `getPointAtLength()` on the real `<path>` each frame, mapped through `getScreenCTM()`, so the curve itself says where the mascot is and a resize needs no listener. The five `<circle>` markers sit on that path within 1.6 user units of a 90-unit-tall viewBox (~1.5px at render scale, inside the circles' own radius 5), so nothing had to be nudged for the walk to genuinely pass over them. The path's x is strictly monotonic, so each marker maps to exactly one arc length — found by binary search at init.
- **Passing a point lights that step.** The corresponding `.route-step` gets `.is-walked`, which carries the full hover treatment (blue heading, `-3px` lift, paragraph revealed). Deliberately a class and **never a synthesized `:hover`**, so the two states cannot contend for one selector and the visitor's own pointer can clear it without fighting. `.is-walked` lives outside the `(hover: hover)` gate that wraps `.route-step:hover` — on a wide touch tablet the walk still runs, and there the paragraphs are already visible anyway.
- **The cursor always wins.** Pointer or focus anywhere on `.route-wrap` freezes the walk, clears `.is-walked`, and drops the sprite to `idle`; leaving resumes it. The mascot must never re-light a step under the hand of someone reading a different one.
- It stops entirely **off-screen**, **below 761px** (where `.route` is `display: none` and the steps become a swipe strip), and under **`prefers-reduced-motion`**, where it is never created and `.route-steps.is-static` opens all five paragraphs instead — they were hidden waiting for a hover that will not arrive. That is `MOTION_SYSTEM.md` §5's poster-frame rule applied to a section rather than a video.
- **It faces the way it is going.** The sprite is not symmetric — 14 of its 21 rows differ when reversed — so it has a real front. Walking right it is mirrored (`scaleX(-1)`); walking back it returns to its natural orientation. The flip is exact with no positional compensation because the body fills all 14 columns of the grid, which makes the box's centre the body's centre too, and `transform-origin` is the default `50%`.
- The mirror needs **two nested elements**: an outer `.route-mate` the script translates every frame, and the inner `.route-mate-sprite` carrying only `scaleX` with its own 260ms transition. A transition on `transform` at one element would brake position as well as facing, turning an exact walk elastic. Direction only changes at the two ends of the row — that is, while the mascot is standing on a point — so the turn reads as turning around before setting off.
- Rhythm is `SPEED` 85 user-units/s between points and `DWELL` 1500ms on each. The sprite switches to `idle` (antennae, blink) while standing and `walk` while moving; it is created `idle` because it starts at rest on the first point, and already mirrored because its first move is rightward.
- `.route` carries `padding-top: 48px` purely as the mascot's headroom: the path's highest point is y=13 and the sprite is 46px tall at `scale="2"`, and `.route-wrap`'s `overflow-x: auto` computes `overflow-y` to `auto`, which would otherwise clip its head.

### Roadmap swipe hint (mobile)

- Below 761px the roadmap becomes a scroll-snap strip, and the dashed line that used to imply continuation is gone. On first entry into the viewport the row slides 26px and returns, **once**, to show it is draggable. It never repeats: a gesture that repeats stops reading as a hint and becomes a tic. It does not fire at all if the visitor already scrolled the strip, or under `prefers-reduced-motion`.
- It is a `transform`, not a real `scrollLeft` write, because scripting scroll position while `scroll-snap-type: x mandatory` is active means fighting the browser's snap.
...
- **Don't** darken the hero shader field's palette stops, or re-enable its cursor reactivity, without redoing the contrast arithmetic recorded in `js/hero-shader.js` — the 20% ceiling is the only thing keeping the `.lede` at AA without a scrim.
